...
class CropsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Reference to the "products/Crops" node in Firebase
        databaseReference = FirebaseDatabase.getInstance().getReference("products").child("Crops")
    ...
    private fun fetchProductsFromFirebase() {
        databaseReference.addValueEventListener(object : ValueEventListener {
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@CropsActivity, "Failed to load data: ${error.message}", Toast.LENGTH_SHORT).show()
                Log.e("CropsActivity", "Database error: ${error.message}")
            }
        })
    }
