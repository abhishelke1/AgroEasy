...
class PoultryActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Reference to the "products/Poultry" node in Firebase
        databaseReference = FirebaseDatabase.getInstance().getReference("products").child("Poultry")
    ...
    private fun fetchProductsFromFirebase() {
        databaseReference.addValueEventListener(object : ValueEventListener {
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@PoultryActivity, "Failed to load data: ${error.message}", Toast.LENGTH_SHORT).show()
                Log.e("PoultryActivity", "Database error: ${error.message}")
            }
        })
    }
